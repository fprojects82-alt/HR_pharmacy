...
export function Sidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-0 bottom-0 z-50 w-64 bg-[var(--card)] border-e border-slate-200 dark:border-slate-800 flex flex-col transition-transform duration-300
      ...
      >
        <div className="flex items-center gap-3 px-4 h-16 border-b border-slate-200 dark:border-slate-800">
          <div className="w-9 h-9 bg-gradient-to-br from-emerald-500 to-emerald-700 rounded-xl flex items-center justify-center flex-shrink-0 shadow-lg shadow-emerald-600/20">
            <span className="text-white font-bold">ن</span>
          </div>
          <span className="font-bold text-slate-900 dark:text-white truncate">{t('appName')}</span>
