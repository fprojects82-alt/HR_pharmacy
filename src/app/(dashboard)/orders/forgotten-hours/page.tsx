...
interface ForgottenHour { id: number; employee_id: number; date: string; start_time: string | null; end_time: string | null; reason: string | null; status: string; hr_decision?: string | null; employees?: { full_name: string } | null }
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {loading ? (
                ...
                <tr key={r.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                  ...
                  {isAdmin && (
                    <td className="px-4 py-3"><ApprovalControl table="forgotten_hours" id={r.id} status={r.status} hrDecision={r.hr_decision} onDone={load} /></td>
                  )}
                </tr>
              ))}
            </tbody>
