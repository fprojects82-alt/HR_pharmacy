...
export default function ForgottenHoursPage() {
  ...
  const fetchRecords = async () => {
    let query = supabase.from('forgotten_hours').select('*, employees(full_name)').order('created_at', { ascending: false })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setShowForm(false); setForm({ date: '', start_time: '', end_time: '', reason: '' }); fetchRecords()
  }

  const handleAction = async (id: number, action: 'approved' | 'rejected') => {
    const { error } = await supabase.from('forgotten_hours').update({ status: action, approved_by: profile?.id, approved_at: new Date().toISOString() }).eq('id', id)
    if (error) { toast.error('فشل'); return }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">ساعات منسية</h1>
        {profile?.employee_id && (
          <button onClick={() => setShowForm(true)} className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2.5 rounded-xl flex items-center gap-2 text-sm font-medium transition"><Plus size={18} /> إضافة ساعات</button>
        )}
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
              {loading ? (
                <tr><td colSpan={7} className="px-4 py-8 text-center text-slate-500">جاري التحميل...</td></tr>
              ) : records.length === 0 ? (
                <tr><td colSpan={7} className="px-4 py-8 text-center text-slate-500">لا يوجد طلبات</td></tr>
              ) : records.map((r) => (
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-md p-6">
            <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-5">إضافة ساعات منسية</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">من ساعة</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">إلى ساعة</label>
