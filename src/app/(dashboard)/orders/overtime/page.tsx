...
export default function OvertimePage() {
  ...
  const fetchRecords = async () => {
    let query = supabase.from('overtime_requests').select('*, employees(full_name)').order('created_at', { ascending: false })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setShowForm(false); setForm({ date: '', hours: '', reason: '' }); fetchRecords()
  }

  const getApprovalField = (): 'hr_approved' | 'area_manager_approved' | 'control_approved' => {
    if (profile?.role === 'control') return 'control_approved'
    if (profile?.role === 'area_manager') return 'area_manager_approved'
    return 'hr_approved'
  }

  const handleApprove = async (id: number) => {
    const field = getApprovalField()
    const updateData: Record<string, unknown> = { [field]: true, [`${field.replace('_approved', '_approved_by')}`]: profile?.id, [`${field.replace('_approved', '_approved_at')}`]: new Date().toISOString() }
    const record = records.find((r) => r.id === id)
    if (record) {
      const hrOk = field === 'hr_approved' ? true : record.hr_approved
      const amOk = field === 'area_manager_approved' ? true : record.area_manager_approved
      const ctrlOk = field === 'control_approved' ? true : record.control_approved
      if (hrOk && amOk && ctrlOk) updateData.status = 'approved'
    }
    const { error } = await supabase.from('overtime_requests').update(updateData).eq('id', id)
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">العمل الإضافي</h1>
        {profile?.employee_id && (
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 dark:bg-slate-700/50">
              <tr>
                ...
                <th className="px-4 py-3 text-right font-medium text-slate-600 dark:text-slate-300">الساعات</th>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-md p-6">
            <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-5">طلب عمل إضافي</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">التاريخ *</label>
                <input type="date" required value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} className="w-full px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 text-sm text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-emerald-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">عدد الساعات *</label>
                <input type="number" required min="0.5" step="0.5" value={form.hours} onChange={(e) => setForm({ ...form, hours: e.target.value })} className="w-full px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 text-sm text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-emerald-500" />
              </div>
              <div>
