...
interface Evaluation {
  id: number; employee_id: number; rate: string; notes: string | null; period_month: number | null; period_year: number | null
  employees?: { full_name: string } | null; evaluation_criteria?: { name: string } | null
}
...
export default function EvaluationsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('evaluations').insert({
      employee_id: Number(form.employee_id), criteria_id: form.criteria_id ? Number(form.criteria_id) : null,
      rate: form.rate, notes: form.notes || null, period_month: Number(form.period_month), period_year: Number(form.period_year),
    })
  ...
  const rateLabel = (r: string) => r === 'excellent' ? t('excellent') : r === 'good' ? t('good') : t('weak')
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">{t('evaluations')}</h1>
        <button onClick={() => setShowForm(true)} className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2.5 rounded-xl flex items-center gap-2 text-sm font-medium transition"><Plus size={18} /> {t('addEvaluation')}</button>
      </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 dark:bg-slate-800/50">
              <tr>
                ...
                <th className="px-4 py-3 text-start font-medium text-slate-600 dark:text-slate-300">{t('criteria')}</th>
                <th className="px-4 py-3 text-start font-medium text-slate-600 dark:text-slate-300">{t('rate')}</th>
                <th className="px-4 py-3 text-start font-medium text-slate-600 dark:text-slate-300">{t('period')}</th>
                <th className="px-4 py-3 text-start font-medium text-slate-600 dark:text-slate-300">{t('notes')}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {loading ? (
                ...
                <tr key={r.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                  ...
                  <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{r.evaluation_criteria?.name || t('general')}</td>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-[var(--card)] rounded-2xl w-full max-w-md p-6">
            <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-5">{t('addEvaluation')}</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div><label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{t('criteria')}</label>
                <select value={form.criteria_id} onChange={(e) => setForm({ ...form, criteria_id: e.target.value })} className={inputCls}>
                  <option value="">{t('general')}</option>
                  {criteria.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
                </select>
              </div>
              <div><label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{t('rate')} *</label>
                <select required value={form.rate} onChange={(e) => setForm({ ...form, rate: e.target.value })} className={inputCls}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{t('month')}</label><input type="number" min="1" max="12" value={form.period_month} onChange={(e) => setForm({ ...form, period_month: e.target.value })} className={inputCls} /></div>
                <div><label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{t('year')}</label><input type="number" value={form.period_year} onChange={(e) => setForm({ ...form, period_year: e.target.value })} className={inputCls} /></div>
              </div>
              <div><label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{t('notes')}</label><textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} className={inputCls} rows={3} /></div>
